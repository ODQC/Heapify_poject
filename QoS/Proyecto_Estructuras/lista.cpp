...
void lista::heapify_down(pnodo actual)
{
	...
	if (actual->anterior && actual->anterior->valor->getPriority() > actual->valor->getPriority()) {
		mayor = actual->anterior;
	}
	if (actual->siguiente && actual->siguiente->valor->getPriority() > mayor->valor->getPriority()) {
		mayor = actual->siguiente;
	}
	if (actual && mayor != actual)
	{
		swap(mayor, actual);
		heapify_down(actual);
	}
